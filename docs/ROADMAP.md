...
Goal: offline · instant · private · builds runnable tools — push GUI + TUI past Codex/Claude Code/CLI on that axis. Autonomous /loop. **✅ H9 COMPLETE (H9.1–H9.9).**
...
- ✅ H9.5 conversation persistence — session.load(messages) seeds prior turns; maker persists the last 50 non-system turns to <id>:history each turn and restores them in loadTool() (so restore()/openTool() auto-resume the conversation); clearConversation() wipes memory + the store key; /clear + /api/clear inherit it. Smoke: 2 turns -> relaunch (new createMaker) restores 4 messages + matching toolId; clear wipes both.
- ✅ H9.6 file-change watcher + doctor --full — startWatcher(store) fs.watches the user's GRANTED folders (recursive, debounced, ignores .rings/node_modules/.git) and fires runHooks(store,'file-change',{path}) so 'file-change' hooks run with MAKER_PATH; wired into TUI + GUI boot/teardown; watch fn injectable. runDoctor({full}) really ensureRuntime()s (download+extract) and reports the binary path, guarded to 'offline: skipped' on network errors; TUI /doctor full. Smoke: injected watch -> hook writes MAKER_PATH; node_modules ignored; doctor --full path + offline skip.
- ✅ H9.7 polish + docs — GUI: transient error TOAST (showToast) instead of raw red error turns for stream/fetch errors; a tool-pane label strip showing the tool NAME (Brief goal) + which model is answering (refreshed on tool-running / model switch). README: new Features list (multi-tool, rewind, vision routing, auto-resume, permissions, hooks, effort, slash menu, one-click models) + honest 'Maker vs cloud agents' section (offline/free/private/instant vs frontier reasoning/context/no-setup; local-first complement with opt-in escalation).
- ✅ H9.8 model orchestration — capability router: pure classifyTask (images→vision; build/make/create/app keywords→code; else chat) + routeModel({task,activeId,installedIds,visionIds}) picks the best INSTALLED model (vision→vision model, code→coder if installed & active isn't one, else active); never requires a non-installed model. GUI generalizes the H9.3 override into a per-model backend cache (ensureModelBackend, started on demand, all stopped on exit) with an honest 'Answered by <model> — <reason>' note when it differs from active; wired into /api/express + /api/chat. TUI /route prints the routing table. Smoke: decision matrix (vision/code/chat × installed sets) + /route.
- ✅ H9.9 optional cloud connect + escalation — cloudInference (OpenAI-compatible streaming, {baseUrl,apiKey,model,label}, injectable fetch; OpenAI/Grok/compatible now, Anthropic+Gemini adapters noted as future); @maker/store cloud.ts (providers + escalation mode 'never'|'auto'|'always', default never, OFF unless a provider is added); pure gaugeComplexity + shouldEscalate; wired into GUI beginRoute (per-request override) and the TUI inference wrapper with an honest '☁ LEFT YOUR DEVICE' note; GUI Settings 'Cloud (optional)' + /api/cloud[/add|remove|mode]; TUI /cloud add|list|remove|mode. Keys stay in ~/.maker, redacted in listings, never logged. Smoke: cloud streams from a fake OpenAI SSE, gauge+escalation matrix, store round-trip + endpoints (no real keys). NOTE deferred: Anthropic /v1/messages + Gemini shapes (need dedicated adapters); complexity gauge is heuristic (length/keywords/verify-fail), not a learned classifier.
